...
MBM fitting
```{r}
m <- glm.mrb(viability~ddg_fold+ddg_trimer, data = scores_viability_all,
                     x.with.me.offsets = c(TRUE, TRUE),
                     me.offsets = ~(abs(errpt_fold))+(abs(errpt_trimer)),
                     minp.formula = ~-1,
                     maxp.formula = ~1,
                     control = list(lower.intercepts = 'xq5',
                                    upper.intercepts = 'xq95'),
                     link = 'logit' ) 
summary(m)
```
...
viab_pred$bi_fit <- predict(m, newdata = viab_pred)
viab_pred$fold_fit <- predict(m,
                              newdata = data.frame(ddg_fold = viab_pred$ddg_fold,
                                                   ddg_trimer = rep(0, nrow(viab_pred))))
viab_pred$bind_fit <- predict(m,
...
ggplot(viab_pred, aes(ddg_fold, ddg_trimer)) +
  ...
  geom_point(data = scores_viability_all, aes(ddg_fold, ddg_trimer, 
                                          color = m$fitted.values, shape = factor(viability))) +
...
#ggsave("../figures/scores.png", width = 9, height = 4, bg = "white")
```

## Viability modeling - Doubles

Create a new doubles dataframe with errors and fit MBM:
```{r}
...
m_doubles <- glm.mrb(viability~ddg_fold_sum+ddg_trimer_sum, data = doubles2_err,
                     x.with.me.offsets = c(TRUE, TRUE),
                     me.offsets = ~errpt_fold_sum+errpt_trimer_sum,
                     minp.formula = ~-1,
                     maxp.formula = ~1,
                     control = list(lower.intercepts = 'xq5',
                                    upper.intercepts = 'xq95'),
...
doubles2_err_pred$bi_fit <- predict(m_doubles,
...
ggplot(doubles2_err_pred, aes(ddg_fold_sum, ddg_trimer_sum)) +
  ...
  geom_point(data = doubles2_err, aes(ddg_fold_sum, ddg_trimer_sum, 
                                          color = m_doubles$fitted.values, shape = factor(viability))) +
  scale_color_stepsn(n.breaks = 10, colors = colors10) +
  # create points to show viable/inviable datapoints
...
x <- seq(start, end, by = 0.05)
prob <- predict(m_doubles, newdata = data.frame(ddg_fold_sum = x,
...
x <- seq(start, end, by = 0.05)
prob <- predict(m_doubles, newdata = data.frame(ddg_fold_sum = rep(0, length(x)),
                                         ddg_trimer_sum = x))
p3 <- 
